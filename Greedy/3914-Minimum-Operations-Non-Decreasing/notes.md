...
Subarray increment operations preserve the relative order of elements, so the minimum number of operations needed to fix any single falling segment is the sum of the drops from the segment peak down to its lowest valley. This insight avoids the need to simulate individual updates, reducing the problem to a single linear pass.
